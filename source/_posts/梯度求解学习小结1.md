...
\end{aligned}$${% endraw %}
其中($\partial L/\partial s)^T$是与$s^T$形状相同的行向量($1 \times K$)。又x本身是列向量($D \times 1$)，所以二者点积为矩阵($D \times K$)，与W形状相同。($\partial L/\partial s)^T$的分量{% raw %}$\partial L/\partial s_j$前面已经求过，即已经求出单样本损失对W的梯度。
