...
L为实数函数，{% raw %}$s_j${% endraw %}为实数，因此{% raw %}$\partial L/\partial s_j${% endraw %}也为实数。
...
这里{% raw %}$W_{\cdot j}${% endraw %}表示矩阵W的第j列，所以：
{% raw %}
...
1. 求{% raw %}$\partial s_j/\partial W${% endraw %}
2. 求{% raw %}$\partial L/\partial s_j${% endraw %}
3. 带入链式法则
4. 求平均损失对W的梯度dW

因为只有loss不同，其他都是一样的，因此只有第二步求{% raw %}$\partial L/\partial s_j${% endraw %}不同。

### 求{% raw %}$\partial L/\partial s_j${% endraw %}
令
{% raw %}

$$p_y = \frac{e^{s_y}}{\sum_m e^{s_m}}$${% endraw %}
有{% raw %}$L=-\log(p_y)${% endraw %}，则
{% raw %}
